...
object InvoicePDFGenerator {
    ...
    fun generatePDF(header: InvoiceHeader, items: List<InvoiceItem>, outputPath: String) {
        ...
        try {
            ...
            itemChunks.forEachIndexed { pageIndex, pageItems ->
                ...
                // 1. Header
                drawHeader(document, contentStream, MARGIN, topY, contentWidth, HEADER_HEIGHT, header.documentType)
                topY -= HEADER_HEIGHT
    ...
    private fun drawHeader(document: PDDocument, stream: PDPageContentStream, x: Float, topY: Float, width: Float, height: Float, title: String) {
        ...
        // 1. Logo (Left Box)
        // Hardcoded path to root/logo.png for now
        ...
        // 3. Invoice Title (Right Box)
        val rightX = x + col1 + col2 + ((width*0.25f)/2)
        val rightBoxCenterY = bottomY + (height / 2)
        PDFUtils.drawCenteredText(stream, "INVOICE", rightX, rightBoxCenterY + 10f, boldFont, 14f)
        PDFUtils.drawCenteredText(stream, "e-Invoice", rightX, rightBoxCenterY - 5f, regFont, 8f)
        PDFUtils.drawCenteredText(stream, "[QR CODE]", rightX, rightBoxCenterY - 20f, PDType1Font.COURIER, 8f)
    }
    ...
    private fun drawDetailsSection(stream: PDPageContentStream, header: InvoiceHeader, x: Float, topY: Float, width: Float, height: Float) {
        ...
        var curY = topY - 12f
        val leftX = x + 5f
        // Max width for text in left column is roughly (width/2) - 10 padding
        ...
        // Take first 3-4 lines of address or wrap it
        if (header.billingAddress.isNotBlank()) {
            ...
            // ensure we don't overlap if address is short
        } else {
            curY -= 10f
        }
        
        // Fixed position for PAN/State/GSTIN at bottom of section to ensure they are always visible
        ...
        // We have limited space here, so strict wrapping is good
    ...
    private fun drawItemRow(stream: PDPageContentStream, item: InvoiceItem?, x: Float, topY: Float, widths: FloatArray, sn: String) {
        ...
        var curX = x
        widths.forEach { w -> curX += w } // Just advance X to keep logic consistent if needed, but not drawing lines
    ...
    private fun drawTotals(stream: PDPageContentStream, items: List<InvoiceItem>, x: Float, topY: Float, widths: FloatArray, height: Float) {
     ...
     private fun drawFooter(stream: PDPageContentStream, header: InvoiceHeader, amount: Double, x: Float, topY: Float, width: Float, height: Float) {
        ...
        // 1. PAN & GSTIN Row (Height approx 25)
        val row1Height = 25f
        ...
        // 2. Amount in Words Row (Height 20)
        val row2Height = 20f
        ...
        // 4. Bank & Signature Row (Remaining Height)
