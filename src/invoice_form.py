...
class InvoiceForm(QtWidgets.QWidget):
    def __init__(self):
        ...
        # map ui widgets (must match objectNames in ui file)
        ...
        # Option A: auto-generate invoice number on form load
        try:
            if self.leInvoiceNo and not self.leInvoiceNo.text().strip():
                self.leInvoiceNo.setText(get_next_invoice_number())
        except Exception:
            self.leInvoiceNo.setText(f"INV-{int(datetime.now().timestamp())}")
    ...
    def _on_cell_changed(self, row, column):
        ...
        try:
            ...
            # columns that affect calculations: rate(qty), qty, taxable, cgst %, sgst %
    ...
    def _build_header_dict(self, total_amount):
        return {
            "invoice_number": self.leInvoiceNo.text().strip(),
    ...
    def save_invoice(self):
        ...
        QMessageBox.information(self, "Saved", f"Invoice saved (ID: {inv_id})")
    ...
    def open_customer_manager(self):
        ...
        # Refresh customer dropdown after closing popup
        self.refresh_customers()
